...
public class ScoreView extends LinearLayout  {
     ...
     */
    public ScoreView(Activity context, CursorView cursorView, AssetManager am, ZoomNotification zn, TapNotification tn) {
        super(context);
    ...
    private void addSystem(final SSystem sys)
    {
        ...
        new Handler(Looper.getMainLooper()).post(new Runnable(){
            ...
            public void run() {
                SystemView sv = new SystemView(getContext(),cursorView, score, sys, ScoreView.this.assetManager, tapNotify);
                addView(sv);
                views.add(sv);
            }
        });
    ...
    public void setScore(final SScore score, final List<Boolean> parts, final float magnification)
    {
        abortLayout(new Runnable(){
            public void run()
            {
                ...
                ScoreView.this.parts = parts;
                ScoreView.this.magnification = magnification;
                ScoreView.this.cursorView.clear();
                if (score != null && getWidth() > 0) {
                    layout(score);
                    setCursorAtBar(0, ScoreView.CursorType.line, 0);
                }
